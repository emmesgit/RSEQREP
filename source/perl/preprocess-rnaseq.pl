...
while (my $entry = <$mta>) {
	...
	unless ($entry=~/subid,samid/) {
		...
		unless (-e $bamFile) {
			...
			addFile($dbh,$sampleId,"","$bamFile",basename($bamFile),sha256sum("$bamFile"),bytes("$bamFile"),'Mapped Reads Bam file')
		}
...
sub downloadFileFromS3{
	...
	my $downloaded_sha256 = sha256sum($encFile);
	my $processId = addProcess($dbh,$sampleId, $cmd,$$bench_ref{return_code},$timeBefore,time());
	addFile($dbh,$sampleId,$processId,$encFile,basename($encFile),$downloaded_sha256,bytes($encFile),'encrypted fastq');
...
sub downloadFileFromSra{
	...
	my $downloaded_sha256 = sha256sum($fqFile);
	my $processId = addProcess($dbh,$sampleId,$cmd,$$bench_ref{return_code},$timeBefore,time());
	addFile($dbh,$sampleId,$processId,$fqFile,basename($fqFile),$downloaded_sha256,bytes($fqFile),'fastq');
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};
}
...
sub decryptFile{			
	...
	my $processId = addProcess($dbh,$sampleId,$cmd,$$bench_ref{return_code},$timeBefore,time());	
	addFile($dbh,$sampleId,$processId,$bamFile,basename($bamFile),sha256sum($bamFile),bytes($bamFile),'fastq');
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};	
}
...
sub executeFastQC {
	...
	addFile($dbh,$sampleId,$processId,$bamFastQcFile,basename($bamFastQcFile),sha256sum($bamFastQcFile),bytes($bamFastQcFile),'FastQC');	
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};
}
...
sub executeBamStatQc {
	...
	my $processId=addProcess($dbh,$sampleId,$cmd,$$bench_ref{return_code},$timeBefore,time());
	addFile($dbh,$sampleId,$processId,$bamQcFile,basename($bamQcFile),sha256sum($bamQcFile),bytes($bamQcFile),'rseqc bam statistics');	
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};
}
...
sub executeBamJunctionQc {
	...
	my $processId=addProcess($dbh,$sampleId,$cmd,$$bench_ref{return_code},$timeBefore,time());	
	addFile($dbh,$sampleId,$processId,$bamJcQcFile,basename($bamJcQcFile),sha256sum($bamJcQcFile),bytes($bamJcQcFile),'reseqc bam junctions');
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};	
}
...
sub executeBamReadDistribution {
	...
	my $processId=addProcess($dbh,$sampleId,$cmd,$$bench_ref{return_code},$timeBefore,time());	
	addFile($dbh,$sampleId,$processId,$bamRdQcFile,basename($bamRdQcFile),sha256sum($bamRdQcFile),bytes($bamRdQcFile),'reseqc bam read distribution');	
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};
}
...
sub executeBamReadGc {
	...
	addFile($dbh,$sampleId,$processId,$bamGcQcFile,basename($bamGcQcFile),sha256sum($bamGcQcFile), bytes($bamGcQcFile),'rseqc bam gc%');
	return $$bench_ref{return_code}			
}
...
sub executeFeatureCountsBam{
	...
	my $processId = addProcess($dbh,$sampleId,$cmd,$$bench_ref{return_code},$timeBefore,time());	
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	addFile($dbh,$sampleId,$processId,$cntFile,basename($cntFile),sha256sum($cntFile),bytes($cntFile),'feature counts');
	return $$bench_ref{return_code};		
}
...
sub star {
	...
	addFile($dbh,$sampleId,$processId,"$bam_file",basename($bam_file),sha256sum("$bam_file"),bytes("$bam_file"),'STAR mapped and unmapped reads bam file');
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};
}
...
sub hisat {
	...
	my $processId = addProcess($dbh,$sampleId,$cmd,$$bench_ref{return_code},$timeBefore,time());
	addFile($dbh,$sampleId,$processId,"$bam_file",basename($bam_file),sha256sum("$bam_file"),bytes("$bam_file"),'HISAT2 mapped and unmapped reads BAM file');
	addBenchmark($dbh,$processId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};
}
...
sub compress_bam {
	...
	my $ProcessId = addProcess($dbh,$sampleId,$Cmd,$$bench_ref{return_code},$timeBefore,time());
	addFile($dbh,$sampleId,$ProcessId,"$cramFile",basename($cramFile),sha256sum("$cramFile"),bytes("$cramFile"),'mapped and unmapped CRAM file');
	addBenchmark($dbh,$ProcessId,$sampleId,$bench_ref);
	return $$bench_ref{return_code};
}

## get sha256sum per file
sub sha256sum {
	my $samid = shift;
	my $sha256Cmd= "sha256sum $samid";
	my $sha256sum=qx($sha256Cmd 2>&1);
	($sha256sum, undef) = split('\s',$sha256sum);
	$sha256sum =~ s/^\s+|\s+$//g ; 
	return $sha256sum;
}
...
sub initDb {
	...
	$dbh->do("create table if not exists file(file_id integer primary key autoincrement, file_name text, file_sha256 text, file_bytes integer, file_type text, 
...
sub addFile {
	my ($dbhr, $sampleId,$processId,$filePath,$samid,$fileSha256,$fileBytes,$fileType)=@_;
	$dbhr->do("insert into file (sample_id,process_id,file_path,file_name,file_sha256,file_bytes,file_type) 
	values($sampleId,$processId,'$filePath','$samid','$fileSha256','$fileBytes','$fileType')");
}
