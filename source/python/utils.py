...
def decryptFile(in_file, openssl, password):
	res = []
	for f in in_file:
		cmd = openssl+' aes-256-cbc -d -pass pass:'+password+' < '+f+' > ' + os.getcwd() + '/' +os.path.basename(f)[:-4]
		#subprocess.run(cmd, shell=True, check=True)
...
def encryptFile(file, openssl, password):
	...
	for f in file:
		cmd = openssl+' aes-256-cbc -pass pass:'+password+' < '+f+' > ' + f + '.enc'
		#subprocess.run(cmd, shell=True, check=True)
		logging_call(cmd, shell=True)
		res.append(f + '.enc')
	return(res)
...
## Source: https://stackoverflow.com/questions/36099331/how-to-grab-all-files-in-a-folder-and-get-their-md5-hash-in-python
def md5(fname):
...
def mergeRSEQC(srcdir):
	...
	return(['sample_metadata.csv','rseqc/bam_qc_parsed.tab', 'rseqc/bam_gc_parsed.tab', 'rseqc/bam_jc_parsed.tab', 'rseqc/bam_rc_parsed.tab', 'feature_counts/fragment_count_matrix.tab.gz','feature_counts/gene_lengths.tab'])
