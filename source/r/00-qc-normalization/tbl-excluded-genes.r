...
if (nrow(dta.tapply)==5) {
	dta.tapply[,1] = c('mitochondrial ribosomal RNA (Mt_rRNA)','mitochondrial transfer RNA (Mt_tRNA)','ribosomal RNA (rRNA)','ribosomal RNA pseudogene (rRNA_pseudogene)','Total')
} else {
...
if(exists('reportAssayFlag')){
	...
	caption = paste0('Number of excluded genes by gene type (',assay.label,').')
}else{
	caption = 'Number of excluded genes by gene type (RNA-Seq).'
}


x = xtable(dta.tapply,align=c('l','l','l'),type='latex',floating='F',label="tab:exgenes",caption=caption);
