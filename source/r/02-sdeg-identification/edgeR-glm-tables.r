		...
		for(t in 1:length(postb.times)) {
			...
			if (file.exists(in.file)) {
				...
				if(exists('reportAssayFlag')){
					...
					caption.short = paste('Genes differentially expressed at ',timel,' compared to pre-treatment (',assay.label,', ',spcLabl,', ',trtLabl,').',sep='');
					caption.long  = paste('Genes differentially expressed at ',timel,' compared to pre-treatment  (',assay.label,', ',spcLabl,', ',trtLabl,'). Sorted by descending absolute $log_{2}$ fold change (Day ',time,' vs. pre-treatment). Gene model summaries and annotations are based on Ensembl Version ',ensembl.version,'.',sep=''); 
