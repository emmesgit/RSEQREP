		...
		for (v in 1:length(trtFlags)) {
			...
			if (nrow(matrx.all)!=0) {
				vps = append(vps,list(grid.grabExpr(c(
						upset.custom(matrx.all,mainbar.y.label='SDEG Intersection Size',sets.x.label='SDEG Set Size',mainbar.y.max=ceiling(max(table(apply(abs(matrx),1,paste,collapse=';')))*1.2),
				...
				colnames(matrx.up) = paste(postb.times.l[-length(postb.times.l)],' (',colSums(abs(matrx.up)),')',sep='')
				...
				colnames(matrx.down) = paste(postb.times.l[-length(postb.times.l)],' (',colSums(abs(matrx.down)),')',sep='')
			...
			if (length(trtFlags)>1) {
				...
				for(t in 1:length(postb.times)) {
					...
					matrx[rownames(matrx) %in% gen.sig,t] = 1
				}
				...
				if (nrow(matrx)!=0) {
					vps = append(vps,list(grid.grabExpr(c(
						upset.custom(matrx,nsets=ncol(matrx),mainbar.y.label='SDEG Intersection Size',sets.x.label='SDEG Set Size',mainbar.y.max=ceiling(max(table(apply(abs(matrx),1,paste,collapse=';')))*1.2),
