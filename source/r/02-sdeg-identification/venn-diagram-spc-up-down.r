...
#
# Program:  venn-diagram-spc-up-down.r
# Version:  RSEQREP 1.1.0
# Author:   Travis L. Jensen and Johannes B. Goll
# Purpose:  Generate Venn diagrams showing overlap in specimen types
	...
	for (v in 1:length(trtFlags)) {
		...
		for(t in 1:length(postb.times)) {
			...
			for(s in 1:length(spcFlags)) {
		 	...
		 	matrx = matrix(rep(0,(length(spcFlags)*length(gen.sig))),ncol=length(spcFlags))
			...
			for(s in 1:length(spcFlags)) {
		 	...
		 	title(main=paste(trtLabl,'\n',timel,sep=''),line=-0.5,xpd=T,cex.main=1.5)	
		 	## Print Up and down regulated diagram
		 	vennDiagramMod(matrx, names=spcLabls,lwd=2,cex=1.4,include=c("up","down"),counts.col=lfc.cols[c(3,1)],show.include=T)
		 	title(main=paste(trtLabl,'\n',timel,sep=''),line=-0.5,xpd=T,cex.main=1.5)
			plot.num = plot.num + 2
		}
		...
		## generate binary matrix
		matrx = matrix(rep(0,(length(spcFlags)*length(gen.sig.time))),ncol=length(spcFlags))
