...
#
# Program:  venn-diagram-time-up-down.r
# Version:  RSEQREP 2.0.0
# Author:   Travis L. Jensen and Johannes B. Goll
# Purpose:  Generate Venn diagrams showing overlap in time points
...
if (length(postb.times) > 1 & length(postb.times) <= 5) {
	...
	for(s in 1:length(spcFlags)) {
		...
		for (v in 1:length(trtFlags)) {
			...
			matrx = matrix(rep(0,(length(postb.times)*length(gen.sig))),ncol=length(postb.times))
			...
			title(main=paste(spcLabl,'\n',trtLabl,sep=''),line=-0.5,xpd=T,cex.main=1.5)	
			## Print Up and down regulated diagram
			vennDiagramMod(matrx, names=postb.times.l[1:length(postb.times)],lwd=2,cex=1.4,include=c("up","down"),counts.col=lfc.cols[c(3,1)],show.include=T)
			title(main=paste(spcLabl,'\n',trtLabl,sep=''),line=-0.5,xpd=T,cex.main=1.5)
			plot.num = plot.num + 2
		}
		...
		## generate binary matrix
		matrx = matrix(rep(0,(length(postb.times)*length(gen.sig.time))),ncol=length(postb.times))
		...
		if (length(trtFlags)>1) {
			...
			title(main=paste(spcLabl,'\n All Treatment Groups',sep=''),line=-0.5,xpd=T,cex.main=1.2)
			plot.num = plot.num + 1
		}
