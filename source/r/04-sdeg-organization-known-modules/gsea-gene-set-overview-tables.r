...
if (file.exists(sets.infile)) {
	...
	if(exists('reportAssayFlag')){
		...
		caption.short = paste('Overview of gene sets used for the enrichment analysis (',assay.label,').',sep='');
		caption.long  = paste('Overview of gene sets used for the enrichment analysis (',assay.label,'). Genes within gene sets are fltered to reflect only those that exist in filtered Ensembl version ',ensembl.version,' anotations obtained using biomaRt.',sep='');
	}else{
