		...
		for(k in 1:length(geneset.types)) {
			...
			for(s in 1:length(spcFlags)) {
				...
				for (v in 1:length(trtFlags)) {
					...
					res.list = tapply(sig.2.types.es$es_or,sig.2.types.es$time,list)
					if(!is.list(res.list)){next} ## checking for data in list
					genesets.es = do.call(rbind.data.frame, res.list)
					...
					if (ncol(genesets.es) > 2) {
						...
						if(exists('reportAssayFlag')){
							...
							RadarLablGsea = c(RadarLablGsea,paste(geneset.type,' (',assay.label,', ',spcLabl,', ',trtLabl,')',sep=''))
						}else{
