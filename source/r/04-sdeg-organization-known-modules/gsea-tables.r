				...
				for(k in 1:length(geneset.types)) {
					...
					if(file.exists(in.file)) {
						...
						if(exists('reportAssayFlag')){
							...
							caption.short = paste('Enriched ',geneset.type,' (',assay.label,', ',spcLabl,', ',trtLabl,', ',postb.timel,')',sep='');
							caption.long  = paste('Enriched ',geneset.type,' (',assay.label,', ',spcLabl,', ',trtLabl,', ',postb.timel,'). Results are sorted by FDR adjusted p-value and Jaccard similarity index. ',labtxt,sep='');
						}else{
