...
#
# Program:  gsea-venn-diagrams-trt.r
# Version:  RSEQREP 1.0.0
# Author:   Travis L. Jensen and Johannes B. Goll
# Purpose:  Generate Venn Diagram plots by treatment group for significant GSEA results
	...
	if (length(trtFlags) > 1 & length(trtFlags) <= 5) {
		...
		for(s in 1:length(spcFlags)) {
			...
			for(k in 1:length(geneset.types)) {
				...
				## Graphing parameters
				par(mar=c(0, 0.2, 2, 0.2),oma=c(0,0,0,0),mfrow=c(5,2),lheight=0.65);
				plot.num = 0
				...
				for(t in 1:length(postb.times)) {
					...
					for (v in 1:length(trtFlags)) {
						...
						if (file.exists(in.file)) {
							gsea.res = read.table(gin.file,sep='\t',stringsAsFactors=F,header=T,quote="",fill=T);
				...
				title(main=paste(spcLabl,', ',postb.times.l[length(postb.times.l)],'\n',geneset.type,sep=''),line=-0.5,xpd=T,cex.main=1.2)
				plot.num = plot.num + 1
