...
fig.dir  = paste(bas.dir,'report','figures',sep='/');
...
glm.sdeg.fold = as.numeric(analysis.config[analysis.config$Name=='glm_model_lfc_cutoff','Value']);
...
aligner_and_version = if(analysis.config[analysis.config$Name=='star_prog','Value']!='0'){
			paste('\\textit{STAR} splice-aware read aligner (Version ',gsub('STAR_','',system('STAR --version',intern=T)),sep='')
		} else {paste('\\textit{Hisat2} splice-aware read aligner (Version ',system("hisat2 --version | head -1 | grep -o -P ' [0-9.]+'",intern=T),sep='')}
...
plotPcaByCell = function(mta,in.dir.pca,spcFlag,spcLabl,visFlag,visLabl) {
	...
	dataEllipse(c1,c2,grid=T,lwd=1,levels = c(1-alpha),xlab='',ylab='',
			...
			group.labels=c(),labels=ids,id.cex=0.4,id.col=1,cex.axis=0.9,cex.lab=0.8,cex=0.4,id.method = "mahal",id.n=3,robust=T);
...
plotMdsByCell = function(mta,in.dir.dist,type,spcFlag,spcLabl,visFlag,visLabl,text) {
	...
	dataEllipse(mds$points,grid=T,lwd=1,levels = c(),xlab="",ylab="",
			groups=as.factor(mta.mds$spct),col=unique(mta.mds$spctc),labels=ids,id.cex=0.4,id.col=1,id.method = "mahal",id.n=3,
