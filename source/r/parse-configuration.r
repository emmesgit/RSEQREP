...
## dtermine if ncores is in the config -- this is an old config -- alert the user.
if ("ncores" %in% workflow.config$Name) {
	cat('ERROR!!! You are using an old configuration file.  ncores is no longer provided in the configuration file.  Please provide the ncores in the command line invocation isung --threads | -@\n')
}
