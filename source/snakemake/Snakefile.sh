...
## Trimmomatic Paired/Unpaird Output Flags
TRIM_OUT_FLAG = ['P', 'U']
...
rule qualityfilter:
    ...
    output:
        temp(expand('rqual_filter/{{sample}}_{pe}{outflag}_qual.fastq.gz', pe=ENDS, outflag=TRIM_OUT_FLAG[TRIM_OUT_IDX])) # Trimmomatic v39
    params:
        sample='{sample}',
    ...
    run:
        ...
        if (int(QUAL_CUTOFF) > 0):
            ...
            if (len(ENDS) == 2):
                output_u = [output[0].replace("1P", "1U"), output[1].replace("2P", "2U")]
                cmd_output = [output[0], output_u[0], output[1], output_u[1]]
                cmd_output = ' '.join(utils.toList(cmd_output))            
...
rule feature_counts:
    ...
    run:
        ...
        if len(ENDS)>1:
            cmd = '%s -B -p -C -T %s -s %s -a %s -o %s %s' % (FEATURECOUNTS, max(1,min(8,NCORES)), STRANDED, ANNOTATIONS_GTF, output, input)
        else:
            cmd = '%s -T %s -s %s -a %s -o %s %s' % (FEATURECOUNTS, max(1,min(8,NCORES)), STRANDED, ANNOTATIONS_GTF, output, input)
