...
## Define final output
if REMOVETEMP and CRAM == 0:
	## Case where we should remove intermediate BAM and we're not saving a CRAM
	...
	expand('rseqc/{sample}_bam_rc.txt', sample=SAMID),
	expand('feature_counts/{sample}_count.tab', sample=SAMID),
	expand('fastqc/{sample}_fastqc.tar.gz', sample=SAMID)]	
else:
	## Otherwise, keep the alignment
	OUTPUT = [expand('rseqc/{sample}_bam_qc.txt', sample=SAMID),
	expand('rseqc/{sample}_bam_gc.txt', sample=SAMID),
	expand('rseqc/{sample}_bam_jc.txt', sample=SAMID),
	expand('rseqc/{sample}_bam_rc.txt', sample=SAMID),
	expand('feature_counts/{sample}_count.tab', sample=SAMID),
	expand('fastqc/{sample}_fastqc.tar.gz', sample=SAMID),
	expand(CRAMFLAG+'/{sample}.'+CRAMFLAG, sample=SAMID)]	
...
rule sam_to_bam:
	...
	run:
		## Run command
		cmd = '%s view -Sbh %s > %s' % (SAMTOOLS, input, output)
		start_time = time.time()
...
rule sort_bam:
	...
	output:
		utils.sortBamOutput(iontorrent=IONTORRENT, cram=CRAM, temp=REMOVETEMP)
	benchmark:
...
rule bam_to_cram:
	...
	run:
		## Run command
		cmd = '%s view -C -@ %s -T %s -o %s %s' % (SAMTOOLS, threads, INDEXSEQ, output, input.bam)
		start_time = time.time()
		bench_obj = utils.logging_call(cmd+utils.returnCode(process='CRAM Compression', sample='{params.sample}', log=LOG_FILE, outfile=utils.toList(output)[0]), shell=True)
		end_time = time.time()
		
		## Add process and output file to output 
		samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
		procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
		sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='cram')
		sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule feature_counts:
	...
	run:
        ...
        if len(ENDS)>1:
            cmd = '%s -B -p -C -T %s -a %s -o %s %s' % (FEATURECOUNTS, threads, ANNOTATIONS_GTF, output, input)
        else:
            cmd = '%s -T %s -a %s -o %s %s' % (FEATURECOUNTS, threads, ANNOTATIONS_GTF, output, input)
		start_time = time.time()
		bench_obj = utils.lsogging_call(cmd+utils.returnCode(process='FeatureCounts', sample='{params.sample}', log=LOG_FILE),shell=True)
		end_time = time.time()
