...
onsuccess:
    ...
    if DOENCRYPT: merged_results = [utils.encryptFile(file=x, openssl=OPENSSL, password=ENCRYPT_PASS) for x in merged_results]
...
rule build_hisat_index:
    ...
    run:
        ## Run command
        cmd = '%s -p %s %s index/hisat2_genome_index && %s %s > annot/splicesites.txt' % (HISAT2BUILD, max(1,min(8,NCORES)), INDEXSEQ, HISAT2SPLICE, ANNOTATIONS_GTF)
        ...
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=SAMID[0], file_type='build_hisat_index')
...
rule build_bowtie_index:
    ...
    run:
        ## Run command
        cmd = '%s -f %s index/bowtie2_genome_index --threads %s' % (BOWTIE2BUILD, INDEXSEQ, max(1,min(8,NCORES)))
        start_time = time.time()
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=SAMID[0])
        procid = sqlite.addProcess(db=LOG_DB, samid=sqlite.getSamid(db=LOG_DB, sample_name=SAMID[0]), cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=SAMID[0], file_type='build_bowtie_index')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule faidx: 
    ...
    run:
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=SAMID[0])
        procid = sqlite.addProcess(db=LOG_DB, samid=sqlite.getSamid(db=LOG_DB, sample_name=SAMID[0]), cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=SAMID[0], file_type='faidx')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule getfile:
    ...
    run:
        ...
        for f in range(len(in_file)):
            start_time = time.time()
            return_code = getfile.getFile(in_file=in_file[f], out_file=utils.toList(output)[f], aws_prog=AWS, fastq_dump_prog=FASTQDUMP, openssl_prog=OPENSSL, pw=DECRYPT_PASS)
            end_time = time.time()
            ...
            sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output)[f], sample_name=params.sample, file_type='getfile')
...
rule trimadapters:
    ...
    run: 
        if TRIM_FP or TRIM_TP:
            ...
            ## Run command
            start_time = time.time()
            bench_obj = trimadapters.trimAdapters(infile=input.fa, outfile=output, adapt5=adapters['fp'], adapt3=adapters['tp'], cutadapt=CUTADAPT)
            end_time = time.time()
            
            ## Add process and output file to output
            samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample[:-2])
            ...
            sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='trimadapters')
            sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
        else:
            utils.touch(output)
...
rule qualityfilter:
    ...
    output:
        temp(expand('rqual_filter/{{sample}}_{pe}{paired}_qual.fastq.gz', pe=ENDS, paired=['','U'][len(ENDS)-1]))
    params:
        sample='{sample}',
    ...
    run:
        ...
        if (int(QUAL_CUTOFF) > 0):
            ...
            cmd = 'java -jar %s %s -threads %s %s %s SLIDINGWINDOW:4:%s' % (TRIMMOMATIC, paired_end_flag, max(1,min(8,NCORES)), cmd_input, cmd_output, QUAL_CUTOFF)                            
        else:
        ...
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='qualityfilter')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule run_hisat:
    ...
    run:
        ...
        ## Run command
        cmd = '%s -p %s -x index/hisat2_genome_index %s %s --known-splicesite-infile annot/splicesites.txt -S %s' % (HISAT2, max(1,min(8,NCORES)), in_fa_str, unmapped_str, output.aln)
        start_time = time.time()
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='hisat2 sam')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)        
...
rule run_bowtie:
    ...
    run:
        ...
        ## Run command
        cmd = '%s --threads %s -x index/bowtie2_genome_index %s --sensitive-local | %s view -h -u -b | %s sort -@ %s - > %s' % (BOWTIE2, NCORES, in_fa_str, SAMTOOLS, SAMTOOLS, max(1,min(8,NCORES)), output)
        start_time = time.time()
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='sorted bam')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)        
...
rule sam_to_bam:
    ...
    run:
        ## Run command
        cmd = '%s view -@ %s -Sbh %s > %s' % (SAMTOOLS, max(1,min(8,NCORES)), input, output)
        start_time = time.time()
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='HISAT2 mapped and unmapped reads BAM file')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule sort_bam:
    ...
    run:
        ## Run command
        cmd = '%s sort -@ %s %s > %s' % (SAMTOOLS, max(1,min(8,NCORES)), input.bam, output)
        start_time = time.time()
        bench_obj = utils.logging_call(cmd+utils.returnCode(process='Sort BAM', sample='{params.sample}', log=LOG_FILE, outfile=utils.toList(output)[0]), shell=True)
        end_time = time.time()
...
rule merge_bam:
    ...
    run:
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='merged sorted bam')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule bam_to_cram:
    ...
    run:
        ## Run command
        cmd = '%s view -C -@ %s -T %s -o %s %s' % (SAMTOOLS, max(1,min(8,NCORES)), INDEXSEQ, output.cram, input.bam)
        start_time = time.time()
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output.cram), sample_name=params.sample, file_type='cram')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
        
        ## Encrypt and/or upload if necessary
        if DOENCRYPT: output.cram = utils.encryptFile(file=utils.toList(output.cram), openssl=OPENSSL, password=ENCRYPT_PASS)
...
rule fastqc: 
    ...
    run:
        ...
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='FastQC')
...
rule bam_qc:
    ...
    run:
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='rseqc bam statistics')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule bam_gc:
    ...
    run:
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='rseqc bam gc%')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule bam_jc:
    ...
    run:
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='rseqc bam junctions')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule read_distribution:
    ...
    run:
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='reseqc bam read distribution')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
...
rule feature_counts:
    ...
    run:
        ## Run command
        cmd = '%s -T %s -a %s -o %s %s' % (FEATURECOUNTS, max(1,min(8,NCORES)), ANNOTATIONS_GTF, output, input)
        start_time = time.time()
        ...
        samid = sqlite.getSamid(db=LOG_DB, sample_name=params.sample)
        procid = sqlite.addProcess(db=LOG_DB, samid=samid, cmd=cmd, return_code=bench_obj.return_code, start_time=start_time, end_time=end_time)
        sqlite.addFile(db=LOG_DB, samid=samid, procid=procid, file=utils.toList(output), sample_name=params.sample, file_type='feature counts')
        sqlite.addBenchmark(db=LOG_DB, procid=procid, samid=samid, bench_obj=bench_obj)
